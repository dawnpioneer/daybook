...
<div class="container">
    <div class="section center-align">
        ...
        <div class="row">
            <g:form name="signupForm" controller="user" action="${isEdit ? 'update' : 'save'}" class="col s12" method="${isEdit ? 'PUT' : 'POST'}">
                <g:hiddenField name="id" value="${user?.id}"/>
                <div class="row">
                    <div class="input-field col s12">
                        <input id="username" name="username" value="${user?.username}"
                               ${isEdit ? "disabled" : ""} type="text" class="validate" required>
                    ...
                    <div class="input-field col s12">
                        <input id="email" name="email" value="${user?.email}"
                               ${isEdit ? "disabled" : ""} type="email" class="validate" required>
                    ...
                    <div class="col s12">
                        ...
                        <a href="${isEdit ? "/user/show/" + sec.loggedInUserInfo(field:'id') : "/"}" class="waves-effect btn a-button">
...
<script>
    ...
    function validatePassword(){
        if(password.value !== confirmPassword.value) {
            confirmPassword.setCustomValidity("密碼與確認密碼不一致。");
        } else {
